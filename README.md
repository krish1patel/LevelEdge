...
The Predictor expects a timezone-aware datetime, e.g. from Python's `zoneinfo` helpers:
...
target_time = datetime(2026, 3, 7, 15, 30, tzinfo=ZoneInfo('US/Eastern'))
predictor = Predictor('AAPL', target_time, '15m', 184.0)

predictor.train_xgb()
probability = predictor.predict_xgb()
print(f"Chance price > $184 by {target_time.time()}: {probability:.1%}")
```
...
- `streamlit run examples/streamlit_option_strike_predictor.py` – Strike Explorer: choose ticker/interval, pick ATM/ITM/OTM strikes, and the Streamlit UI reports LevelEdge probabilities, direction suggestions, and metrics per strike.
- `streamlit run examples/streamlit_option_breakevens.py` – Breakeven Assistant: enter ticker/interval/expiry, pick ITM/ATM/OTM, and get the predicted probability plus breakeven price for each selected strike.
